...
/* ---------- HERO: IMAGE -> VIDEO REVEAL ----------
   Loads as the static poster only (see .hero-bg-photo) — no autoplay on
   page load, no bandwidth spent on a video that visitor might never
   scroll past. Starts on whichever comes first: a real interaction
   (hover/mouse move, tap, scroll, key press) anywhere on the page, or a
   0.001-second fallback timer — effectively immediate, firing on
   arrival for anyone who doesn't happen to interact first, while a
   genuine interaction that lands in that same instant still goes
   through the same activate() path (activated is a one-shot guard, so
   whichever fires first wins and the other is a no-op). Once
   #heroVideo has a decodable first frame, .is-active
   fades it in over the photo with a slight zoom-out/brightness lift
   (see the CSS transition on .hero-bg-video). Requiring a genuine
   gesture before calling play() (when that's what triggers it) is also
   what makes autoplay reliable on iOS Safari, which otherwise blocks
